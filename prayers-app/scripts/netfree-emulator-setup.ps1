...
try {
  ...
  if ($mitm -eq 0) { Die "Found no NetFree intermediate in the live chain ($j certs seen). Are you actually behind Netfree?" }
...
Verify:
  adb logcat -d | Select-String cr_CommandLine
  adb logcat -c ; <sign in from the app> ; adb logcat -d | Select-String "net_error -150"

Then: Settings > Passwords & accounts > Add account -> add a Google account.
